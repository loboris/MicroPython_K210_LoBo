...
#srv.Start(threaded=False, stackSize=4096, debug=True)

if network.wifi_active():
    if _thread.pystack() > 0:
        #srv.Start(threaded=srv_run_in_thread, stackSize=2048)
        srv.Start(threaded=srv_run_in_thread, stackSize=8192, debug=True)
    else:
        #srv.Start(threaded=srv_run_in_thread, stackSize=4096)
        srv.Start(threaded=srv_run_in_thread, stackSize=8192, debug=True)
else:
...
'''
Test the server from web browser:
---------------------------------
- get main page
http://<MicroPython's_IPaddr>

- test form with post parameters
http://<MicroPython's_IPaddr>/TEST

- passing some parameters
http://<MicroPython's_IPaddr>/edit
http://<MicroPython's_IPaddr>/edit/123
http://<MicroPython's_IPaddr>/edit/123/abc/bar

- execute python script
http://<MicroPython's_IPaddr>/test.pyhtml

- websockets test
http://<MicroPython's_IPaddr>/wstest.html
'''
