...
def cat(src_filename, dst_file):
    ...
    if dev is None:
        ...
        return dev.remote(maix_send_file_to_host, dev_filename, dst_file, filesize,
                          xfer_func=maix_recv_file_from_remote)
...
def cp(src_filename, dst_filename):
    ...
    if dst_dev is None:
        ...
        with open(dst_dev_filename, 'wb') as dst_file:
            return src_dev.remote(maix_send_file_to_host, src_dev_filename, dst_file,
                                  filesize, xfer_func=maix_recv_file_from_remote)
    if src_dev is None:
        # Copying from host to remote
        with open(src_dev_filename, 'rb') as src_file:
            return dst_dev.remote(maix_recv_file_from_host, src_file, dst_dev_filename,
                                  filesize, xfer_func=maix_send_file_to_remote)
    ...
    if src_dev.remote(maix_send_file_to_host, src_dev_filename, host_temp_file,
                      filesize, xfer_func=maix_recv_file_from_remote):
        host_temp_file.seek(0)
        return dst_dev.remote(maix_recv_file_from_host, host_temp_file, dst_dev_filename,
                              filesize, xfer_func=maix_send_file_to_remote)
    return False
...
def set_time(rtc_time, tzone):
    try:
        import utime
        _ = utime.mktime(rtc_time, tz=tzone, setrtc=True)
    except:
        pass
...
def maix_recv_file_from_host(src_file, dst_filename, filesize, dst_mode='wb'):
    """Function which runs on the pyboard. Matches up with maix_send_file_to_remote."""
    import os
    try:
        res = os.get_file(dst_filename, filesize)
        return res
    except:
        return False
...
def maix_send_file_to_remote(dev, src_file, dst_filename, filesize, dst_mode='wb'):
    """Intended to be passed to the `remote` function as the xfer_func argument.
       Matches up with recv_file_from_host.
    """
    bytes_remaining = filesize
    ...
    while bytes_remaining > 0:
        ...
        if ack == b'\x07' or ack == b'\x06':
            ...
            if bytes_remaining > 0:
                sys.stderr.write("\nabort requested from remote [{}]\n".format(ack[0]))
                break
        else:
            if bytes_remaining > 0:
                sys.stderr.write("\ntimed out or error in sending file to remote [{}]\n".format(ack))
                break
    sys.stdout.write("\n")
    dev.timeout = save_timeout


def maix_recv_file_from_remote(dev, src_filename, dst_file, filesize):
    """Intended to be passed to the `remote` function as the xfer_func argument.
       Matches up with maix_send_file_to_host.
    """
    bytes_remaining = filesize
    ntry = 3
    save_timeout = dev.timeout
    ...
    while bytes_remaining > 0:
        read_size = min(bytes_remaining, 1024)
        while ntry > 0:
            read_buf = dev.read(read_size+2)
            if len(read_buf) != read_size+2:
                sys.stderr.write("\nwrong block size received: {}, expected {} [{}]\n".format(len(read_buf), read_size+2, read_buf))
                ntry = 0
                continue
            ...
            if crc == rcrc:
                ...
                sys.stdout.write("\r<R< {0:.2f}%".format((filesize-bytes_remaining) / filesize * 100))
                sys.stdout.flush()
                dev.write(b'\x07')   # ASCII ACK is 0x07, repeat
                ntry -= 1
        if ntry == 0:
            sys.stderr.write("\ntimed out or error in receiving file from remote\n")
            dev.write(b'\x08')   # ASCII ACK is 0x08, abort
            bytes_remaining = 0
            continue
        bytes_remaining -= read_size
    if ntry > 0:
...
def maix_send_file_to_host(src_filename, dst_file, filesize):
    """Function which runs on the pyboard. Matches up with recv_file_from_remote."""
    import os
    try:
        res = os.send_file(src_filename, filesize);
        return res
    except:
        return False
...
def test_unhexlify():
    ...
    try:
        machine.loglevel(machine.LOG_WARN)
...
class Device(object):
    ...
    def remote(self, func, *args, xfer_func=None, **kwargs):
        ...
        try:
            ...
            self.check_pyb()
            output, _ = self.pyb.follow(timeout=60)
            self.check_pyb()
    ...
    def sync_time(self):
        ...
        self.remote(set_time, (now.tm_year, now.tm_mon, now.tm_mday,
                               now.tm_hour, now.tm_min, now.tm_sec, 0, 0), tz)
        return now
    ...
    def postcmd(self, stop, line):
        if self.stdout != self.smart_stdout:
            if self.redirect_dev is not None:
                ...
                self.stdout.seek(0)
                self.redirect_dev.remote(maix_recv_file_from_host, self.stdout,
                                         self.redirect_filename, filesize,
                                         dst_mode=self.redirect_mode,
                                         xfer_func=maix_send_file_to_remote)
            self.stdout.close()
