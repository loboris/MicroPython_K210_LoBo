...
## other keys
    ...
    def __init__(self, tab_size, undo_limit):
        ...
        self.autoindent = "y"
        self.replc_pattern = ""
        self.comment_char = "\x23 " ## for #
        self.word_char = "_\\" ## additional character in a word
        ...
        if key == KEY_NONE: ## character to be added
            ...
            if self.mark is not None:
                ...
                self.content[self.cur_line] = l + (
                    ...
                    if self.autoindent == "y" and self.col > 0
            ...
            ni = len(l.split(self.comment_char.strip())[0].rstrip())
            ns = self.spaces(l)
            ...
            pat = self.line_edit("Find: ", self.find_pattern, "_")
            if pat:
            ...
            self.autoindent, self.case, self.tab_size, self.comment_char, self.write_tabs), "")
            try:
                res =  [i.lstrip().lower() for i in pat.split(",")]
                if res[0]: self.autoindent = 'y' if res[0][0] == 'y' else 'n'
                if res[1]: self.case     = 'y' if res[1][0] == 'y' else 'n'
                if res[2]: self.tab_size = int(res[2])
                if res[3]: self.comment_char = res[3]
            ...
            count = 0
            pat = self.line_edit("Replace: ", self.find_pattern, "_")
            if pat:
                rpat = self.line_edit("With: ", self.replc_pattern, "_")
            ...
            ni = len(self.comment_char)
            for i in range(lrange[0],lrange[1]):
                ns = self.spaces(self.content[i])
                if self.content[i][ns:ns + ni] == self.comment_char:
                    self.content[i] = ns * " " + self.content[i][ns + ni:]
                else:
                    self.content[i] = ns * " " + self.comment_char + self.content[i][ns:]
        elif key == KEY_REDRAW:
            self.redraw(True)
...
## Read file into content
    def get_file(self, fname):
        ...
        if is_file == 2:
            ...
            self.fname = fname.rstrip('/') + "new_file.txt"
        else:
            self.content = ["New file", ""]
        if fname in ('', '.', '..'):
            self.fname = os.getcwd() + "new_file.txt"
