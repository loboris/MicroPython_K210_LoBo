...
static int _clear_connection_info(esp_http_client_handle_t client);
/**
 * Default settings
 */
...
static const char *DEFAULT_HTTP_USER_AGENT = "MicroPython HTTP Client/1.0";
...
static int http_dispatch_event(esp_http_client_t *client, esp_http_client_event_id_t event_id, void *data, int len)
{
...
static int http_on_message_begin(http_parser *parser)
{
    ...
    if (transport_debug) LOGD(TAG, "on_message_begin");
...
{
    if (transport_debug) LOGD(TAG, "http_on_url");
    return 0;
}
...
static int http_on_header_value(http_parser *parser, const char *at, size_t length)
{
    ...
    if (transport_debug) LOGD(TAG, "HEADER=%s:%s", client->current_header_key, client->current_header_value);
    ...
    vPortFree(client->current_header_key);
    vPortFree(client->current_header_value);
...
static int http_on_headers_complete(http_parser *parser)
{
    ...
    if (transport_debug) LOGM(TAG, "http_on_headers_complete, status=%d, offset=%d, nread=%d", parser->status_code, client->response->data_offset, parser->nread);
...
{
    esp_http_client_t *client = parser->data;
    if (transport_debug) LOGD(TAG, "http_on_body %lu", length);
...
{
    if (transport_debug) LOGD(TAG, "http_on_message_complete, parser=%p", parser);
    esp_http_client_handle_t client = parser->data;
...
{
    if (transport_debug) LOGD(TAG, "http_on_chunk_complete");
    return 0;
}
...
int esp_http_client_delete_header(esp_http_client_handle_t client, const char *key)
{
    return http_header_delete(client->request->headers, key);
}

static int _set_config(esp_http_client_handle_t client, const esp_http_client_config_t *config)
{
    ...
    if (config->path) {
        client->connection_info.path = string_dup(config->path);
    } else {
        client->connection_info.path = string_dup(DEFAULT_HTTP_PATH);
    }
    ...
    if (config->host) {
        client->connection_info.host = string_dup(config->host);
    ...
    if (config->query) {
        client->connection_info.query = string_dup(config->query);
    ...
    if (config->username) {
        client->connection_info.username = string_dup(config->username);
    ...
    if (config->password) {
        client->connection_info.password = string_dup(config->password);
...
static int _clear_connection_info(esp_http_client_handle_t client)
{
    if (client->connection_info.path) vPortFree(client->connection_info.path);
    if (client->connection_info.host) vPortFree(client->connection_info.host);
    if (client->connection_info.query) vPortFree(client->connection_info.query);
    if (client->connection_info.username) vPortFree(client->connection_info.username);
    if (client->connection_info.password) {
        memset(client->connection_info.password, 0, strlen(client->connection_info.password));
        vPortFree(client->connection_info.password);
    }
    if (client->connection_info.scheme) vPortFree(client->connection_info.scheme);
    if (client->connection_info.url) vPortFree(client->connection_info.url);
    memset(&client->connection_info, 0, sizeof(connection_info_t));
    return 0;
}

static int _clear_auth_data(esp_http_client_handle_t client)
{
    if (client->auth_data == NULL) {
        return -1;
    }

    if (client->auth_data->method) vPortFree(client->auth_data->method);
    if (client->auth_data->realm) vPortFree(client->auth_data->realm);
    if (client->auth_data->algorithm) vPortFree(client->auth_data->algorithm);
    if (client->auth_data->qop) vPortFree(client->auth_data->qop);
    if (client->auth_data->nonce) vPortFree(client->auth_data->nonce);
    if (client->auth_data->opaque) vPortFree(client->auth_data->opaque);
    memset(client->auth_data, 0, sizeof(esp_http_auth_data_t));
    return 0;
}

static int esp_http_client_prepare(esp_http_client_handle_t client)
{
...
esp_http_client_handle_t esp_http_client_init(const esp_http_client_config_t *config)
{
    ...
    client                         = pvPortMalloc(sizeof(esp_http_client_t));
    if (!client) goto err_exit;
    memset(client, 0, sizeof(esp_http_client_t));
    client->parser                 = pvPortMalloc(sizeof(struct http_parser));
    if (!client->parser) goto err_exit;
    memset(client->parser, 0, sizeof(struct http_parser));
    client->parser_settings        = pvPortMalloc(sizeof(struct http_parser_settings));
    if (!client->parser_settings) goto err_exit;
    memset(client->parser_settings, 0, sizeof(struct http_parser_settings));
    client->auth_data              = pvPortMalloc(sizeof(esp_http_auth_data_t));
    if (!client->auth_data) goto err_exit;
    memset(client->auth_data, 0, sizeof(esp_http_auth_data_t));
    client->request                = pvPortMalloc(sizeof(esp_http_data_t));
    if (!client->request) goto err_exit;
    memset(client->request, 0, sizeof(esp_http_data_t));
    client->request->headers       = http_header_init();
    if (!client->request->headers) goto err_exit;
    client->request->buffer        = pvPortMalloc(sizeof(esp_http_buffer_t));
    if (!client->request->buffer) goto err_exit;
    memset(client->request->buffer, 0, sizeof(esp_http_buffer_t));
    client->response               = pvPortMalloc(sizeof(esp_http_data_t));
    if (!client->response) goto err_exit;
    memset(client->response, 0, sizeof(esp_http_data_t));
    client->response->headers      = http_header_init();
    if (!client->response->headers) goto err_exit;
    client->response->buffer       = pvPortMalloc(sizeof(esp_http_buffer_t));
    if (!client->response->buffer) goto err_exit;
    memset(client->response->buffer, 0, sizeof(esp_http_buffer_t));

    client->transport_list = transport_list_init();
    tcp = transport_tcp_init();
    _success = (client->transport_list && tcp &&
                   (transport_set_default_port(tcp, DEFAULT_HTTP_PORT) == 0) &&
                   (transport_list_add(client->transport_list, tcp, "http") == 0)
               );
    if (!_success) {
        if (transport_debug) LOGE(TAG, "Error initialize transport");
    ...
    if (_set_config(client, config) != 0) {
        if (transport_debug) LOGE(TAG, "Error set configurations");
        esp_http_client_cleanup(client);
        return NULL;
    }

    client->request->buffer->data  = pvPortMalloc(client->buffer_size);
    client->response->buffer->data = pvPortMalloc(client->buffer_size);

    if ((client->request->buffer->data == NULL) || (client->response->buffer->data == NULL)) {
        if (transport_debug) LOGE(TAG, "Allocation failed");
        esp_http_client_cleanup(client);
        return NULL;
    }
    ...
    if (!_success) {
        if (transport_debug) LOGE(TAG, "Error set default configurations");
        esp_http_client_cleanup(client);
        return NULL;
    }
...
err_exit:
    if (transport_debug) LOGE(TAG, "Error allocate memory");
    esp_http_client_cleanup(client);
    return NULL;
}
...
int esp_http_client_cleanup(esp_http_client_handle_t client)
{
    ...
    if (client->request->buffer->data) vPortFree(client->request->buffer->data);
    if (client->request->buffer) vPortFree(client->request->buffer);
    if (client->request) vPortFree(client->request);
    http_header_destroy(client->response->headers);
    if (client->response->buffer->data) vPortFree(client->response->buffer->data);
    if (client->response->buffer) vPortFree(client->response->buffer);
    if (client->response) vPortFree(client->response);

    if (client->parser) vPortFree(client->parser);
    if (client->parser_settings) vPortFree(client->parser_settings);
    _clear_connection_info(client);
    _clear_auth_data(client);
    if (client->auth_data) vPortFree(client->auth_data);
    if (client->current_header_key) vPortFree(client->current_header_key);
    if (client->location) vPortFree(client->location);
    if (client->auth_header) vPortFree(client->auth_header);
    if (client) vPortFree(client);
    return 0;
}

static int esp_http_check_response(esp_http_client_handle_t client)
{
    char *auth_header = NULL;

    if (client->redirect_counter >= client->max_redirection_count || client->disable_auto_redirect) {
        if (transport_debug) LOGE(TAG, "Error, reach max_redirection_count count=%d", client->redirect_counter);
        return ESP_ERR_HTTP_MAX_REDIRECT;
    }
    switch (client->response->status_code) {
        case HttpStatus_MovedPermanently:
        case HttpStatus_Found:
            if (transport_debug) LOGI(TAG, "Redirect to %s", client->location);
        ...
        case HttpStatus_Unauthorized:
            ...
            if (auth_header) {
                ...
                if (transport_debug) LOGD(TAG, "UNAUTHORIZED: %s", auth_header);
                client->redirect_counter ++;
                if (http_utils_str_starts_with(auth_header, "Digest") == 0) {
                    if (transport_debug) LOGD(TAG, "type = Digest");
                    client->connection_info.auth_type = HTTP_AUTH_TYPE_DIGEST;
                } else if (http_utils_str_starts_with(auth_header, "Basic") == 0) {
                    if (transport_debug) LOGD(TAG, "type = Basic");
                    client->connection_info.auth_type = HTTP_AUTH_TYPE_BASIC;
                } else {
                    client->connection_info.auth_type = HTTP_AUTH_TYPE_NONE;
                    if (transport_debug) LOGE(TAG, "This authentication method is not supported: %s", auth_header);
                    break;
                }

                _clear_auth_data(client);

                client->auth_data->method = string_dup(HTTP_METHOD_MAPPING[client->connection_info.method]);
                ...
                client->connection_info.auth_type = HTTP_AUTH_TYPE_NONE;
                if (transport_debug) LOGW(TAG, "This request requires authentication, but does not provide header information for that");
            }
    }
    return 0;
}
...
int esp_http_client_set_url(esp_http_client_handle_t client, const char *url)
{
    ...
    if (client == NULL || url == NULL) {
        if (transport_debug) LOGE(TAG, "client or url must not NULL");
    ...
    if (parser_status != 0) {
        if (transport_debug) LOGE(TAG, "Error parse url %s", url);
        return K210_ERR_INVALID_ARG;
    }
    ...
    if (purl.field_data[UF_HOST].len) {
        ...
        HTTP_MEM_CHECK(TAG, client->connection_info.host, return K210_ERR_NO_MEM);
    }
    ...
    if (old_host && client->connection_info.host
        ...
        if (transport_debug) LOGD(TAG, "New host assign = %s", client->connection_info.host);
    ...
    if (purl.field_data[UF_SCHEMA].len) {
        ...
        HTTP_MEM_CHECK(TAG, client->connection_info.scheme, return K210_ERR_NO_MEM);
        ...
        if (user_info) {
            ...
            if (password) {
                *password = 0;
                ...
                HTTP_MEM_CHECK(TAG, client->connection_info.password, return K210_ERR_NO_MEM);
            }
            http_utils_assign_string(&client->connection_info.username, username, 0);
            HTTP_MEM_CHECK(TAG, client->connection_info.username, return K210_ERR_NO_MEM);
            vPortFree(user_info);
        } else {
            return K210_ERR_NO_MEM;
        }
    } else {
        vPortFree(client->connection_info.username);
        vPortFree(client->connection_info.password);
    ...
    HTTP_MEM_CHECK(TAG, client->connection_info.path, return K210_ERR_NO_MEM);
    ...
    if (purl.field_data[UF_QUERY].len) {
        ...
        HTTP_MEM_CHECK(TAG, client->connection_info.query, return K210_ERR_NO_MEM);
    } else if (client->connection_info.query) {
        vPortFree(client->connection_info.query);
...
static int esp_http_client_get_data(esp_http_client_handle_t client)
{
    ...
    if (transport_debug) LOGM(TAG, "data_process=%d, content_length=%d", client->response->data_process, client->response->content_length);
...
int esp_http_client_read(esp_http_client_handle_t client, char *buffer, int len)
{
    ...
    while (need_read > 0 && is_data_remain) {
        ...
        if (transport_debug) LOGD(TAG, "is_data_remain=%d, is_chunked=%d", is_data_remain, client->response->is_chunked);
        if (!is_data_remain) {
            break;
        }
...
int esp_http_client_perform(esp_http_client_handle_t client)
{
    int err;
    do {
        ...
        if (client->post_data && client->post_len) {
            if (esp_http_client_write(client, client->post_data, client->post_len) <= 0) {
                if (transport_debug) LOGE(TAG, "Error upload data");
        ...
        if ((err = esp_http_check_response(client)) != 0) {
            if (transport_debug) LOGE(TAG, "Error response");
            return err;
        }
        ...
        if (client->connection_info.method != HTTP_METHOD_HEAD) {
            while (client->response->is_chunked && !client->is_chunk_complete) {
                if (esp_http_client_get_data(client) <= 0) {
                    if (transport_debug) LOGD(TAG, "Read finish or server requests close");
                    break;
                }
            }
        ...
        if (!http_should_keep_alive(client->parser)) {
            if (transport_debug) LOGD(TAG, "Close connection");
            esp_http_client_close(client);
        } else {
...
// LoBo, added
int esp_http_client_perform_response(esp_http_client_handle_t client)
{
    int err;
    if (esp_http_client_fetch_headers(client) < 0) {
        return ESP_ERR_HTTP_FETCH_HEADER;
...
int esp_http_client_open(esp_http_client_handle_t client, int write_len)
{
    ...
    if (client->state == HTTP_STATE_UNINIT) {
        if (transport_debug) LOGE(TAG, "Client has not been initialized");
        return K210_ERR_INVALID_STATE;
    }

    if ((err = esp_http_client_prepare(client)) != 0) {
        if (transport_debug) LOGE(TAG, "Failed to initialize request data");
        esp_http_client_close(client);
        return err;
    }

    if (client->state < HTTP_STATE_CONNECTED) {
        if (transport_debug) LOGD(TAG, "Begin connect to: %s://%s:%d", client->connection_info.scheme, client->connection_info.host, client->connection_info.port);
        client->transport = transport_list_get_transport(client->transport_list, client->connection_info.scheme);
        if (client->transport == NULL) {
            if (transport_debug) LOGE(TAG, "No transport found");
        ...
        if (transport_connect(client->transport, client->connection_info.host, client->connection_info.port, client->timeout_ms) < 0) {
            if (transport_debug) LOGE(TAG, "Connection failed, sock < 0");
    ...
    if (first_line > client->buffer_size) {
        if (transport_debug) LOGE(TAG, "Out of buffer");
        return ESP_ERR_HTTP_CONNECT;
    }
    ...
    while ((header_index = http_header_generate_string(client->request->headers, header_index, client->request->buffer->data + first_line, &wlen))) {
        ...
        if (transport_debug) LOGD(TAG, "Write header[%d]: %s", header_index, client->request->buffer->data);
        if (transport_write(client->transport, client->request->buffer->data, wlen, client->timeout_ms) <= 0) {
            if (transport_debug) LOGE(TAG, "Error write request");
            esp_http_client_close(client);
            return ESP_ERR_HTTP_WRITE_DATA;
        }
...
int esp_http_client_set_post_field(esp_http_client_handle_t client, const char *data, int len)
{
    int err = 0;
    client->post_data = (char *)data;
    client->post_len = len;
    if (transport_debug) LOGD(TAG, "set post file length = %d", len);
    if (client->post_data) {
