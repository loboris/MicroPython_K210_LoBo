...
char *http_auth_basic(const char *username, const char *password)
{
    ...
    if (!base64_encode((unsigned char *)user_info, strlen(user_info), NULL, &n)) {
        vPortFree(user_info);
        return NULL;
    }
    digest = pvPortMalloc(6 + n + 1);
    HTTP_MEM_CHECK(TAG, digest, goto _basic_exit);
    strcpy(digest, "Basic ");
    if (!base64_encode((unsigned char *)user_info, strlen(user_info), (unsigned char *)digest + 6, &n)) {
        vPortFree(digest);
        vPortFree(user_info);
        return NULL;
    }
